...
> 구현 이슈: `.scratch/archdraw-skill/issues/01~07` 작성됨. 순서 = 01 → 03 → 04~07 → 02 도그푸딩(맨 마지막, 2026-09-14 그릴). 검수 인계는 `review.md`.
...
1. **상황 → 그림 라우팅.** 백엔드 실무 21개 상황마다 답하는 질문 한 문장, 어떤 다이어그램 종류를, 어떤 요소가 빠지면 안 되는지.
2. **캔버스 우선.** 에이전트는 행동 전에 캔버스를 보고, 내가 바꾼 건 뒤엎지 않는다. 텍스트 설명 없이 그림으로 왕복.
3. **줌 레벨과 연결.** 시스템 → 배포 단위 → 모듈 → 레이어 → 경계 타입. 확대는 실제 구조를 따르며, 중복 단계 생략과 공용 모듈 그림의 공유 참조를 허용한다. 레벨은 스택 무관, 근거 추출만 스택별 어댑터.
...
- **변경안 표기** (2026-09-12 그릴 확정, 스토리 32): 수정·기능 추가 논의는 기본이 지금 구조 그림 위에 바뀌는 부분만 새 색. 구조가 크게 바뀌면 지금/바꾼 구조 두 그림을 나란히(그림 복사 필요 — 검수 R02와 엮임); 마이그레이션 행은 이를 단계 수만큼으로 확장. 코드에 아직 없는 선은 근거 태그 `설계`, 실선 + 새 색 — `코드` 태그면 선마다 "근거 없음"이 쌓이기 때문.
...
- 상황 21행. 행마다 **답하는 질문 한 문장** + 그림 종류 + 필수요소 + 라우팅 예외(이 조건이면 다른 행으로 보낸다). 행을 나누는 기준은 **답하는 질문이 다른가**다 — 그림 종류가 같아도 질문이 다르면 다른 행(API 동작·인증·권한은 둘 다 시퀀스). 상황 이름은 사용자 처지("처음 보는 서비스")가 아니라 답하는 질문 기준("시스템 개요").
...
- **마이그레이션** — 질문: 지금 것에서 새 것으로 어떤 단계를 거쳐 옮기고, 각 단계에서 무엇이 어디를 읽고 쓰며, 어디까지 되돌릴 수 있나? · 그림: **단계 수만큼 같은 종류 그림을 나란히**(before/after 두 장 폐기), 각 장은 그 단계의 읽기·쓰기 경로만, 단계 이름이 frame 제목. 선은 대부분 코드에 없어 근거 태그 `설계` · 필수요소: ① 단계 목록과 현재 단계 ② 단계별 읽기·쓰기 경로(읽기 전환과 쓰기 전환은 다른 단계) ③ 이중 쓰기 구간 ④ 원본 이동 시점 ⑤ 정합성 검증 방법("없음 확인" = 검증 없는 컷오버) ⑥ 롤백 가능 구간과 만료 조건 ⑦ 정리 단계 · 라우팅 예외: 다 끝난 뒤 구조 → 해당 구조 행 / 배포 단위 교체 → 배치도(배포 롤백 ≠ 데이터 롤백) / 코드 인터페이스 변경만 → API 계약 / 지금 상태(원본·계보) → 데이터 흐름.
...
- 확대는 실제 구조를 따르며, 중복 단계 생략과 공용 모듈 그림의 공유 참조를 허용한다. 박스 라벨에 확대할 그림 참조를 남긴다. 공용 모듈의 내부 그림은 하나로 두고 사용하는 배포 단위들이 함께 참조한다. 내부 탐색은 사용자 발화로 요청하며, 에이전트는 캔버스에 해당 내부 그림이 있으면 그 위치로 화면을 이동·확대하고 없으면 새로 그린다(03 담당). 박스 클릭 탐색을 요구하지 않는다. 파일명·그림 참조의 저장 및 재로드 계약은 R07 잔여 쟁점이다.
...
- **프로젝트 루트·저장소 범위 (2026-09-14 그릴 확정, 검수 R03):** `session start`가 cwd에서 git 루트를 찾아 세션의 프로젝트 루트로 고정한다(없으면 cwd, `--project <경로>`로 덮어쓰기, 출력에 잡힌 루트 표시). 이후 호출은 `--session` 키만으로 같은 프로젝트를 가리켜 하위 폴더·스킬 폴더에서 불러도 근거 검사(7-3)·export 허용 폴더(7-5b)가 그 루트 기준이다. 스냅샷은 프로젝트별 하위 폴더(식별자 = 루트 경로 해시)에 두고, 같은 프로젝트의 세션들과 다음 날 새 세션이 공유한다 — 캔버스는 세션별(7-8), 스냅샷은 프로젝트별. 같은 이름 저장은 `snapshot save`·`export` 모두 기본 거부(있음·만든 시각 출력), `--force`로만 덮어쓴다 — §3.5 "안 지움"을 파일에도 적용. 기각: 매 호출 cwd(하위 폴더 호출 시 어긋남) / `--project` 매번(인자 증가) / 세션별 스냅샷 폴더(세션 키가 사라져 다음 날 못 찾음, 스토리 17 위반) / 조용한 덮어쓰기·자동 접미사. 담당: 루트 고정은 04, 저장소 범위·동명 정책은 06.
- **저장 범위·파일 단위 (2026-09-14 그릴 확정, 검수 R07 일부):** "저장해"는 캔버스 전체를 현재 그림들의 배치와 함께 `.excalidraw` 파일 하나로 `docs/architecture/`에 저장한다. "주문 그림만 저장해"처럼 대상을 지정하면 그 그림만 저장한다. 범위는 매번 되묻지 않고, 지정 대상이 애매할 때만 확인한다. 발화 해석은 03, 전체·선택 export와 배치 보존은 06 담당. 파일명과 미확인 필수요소의 저장 조건은 R07에서 이어서 정한다.
...
- **이 스펙의 판단 규칙은 검증되지 않았다.** 세션 중 "남길지 먼저 정하라"가 "기본 안 남김, 말하면 승격"으로 뒤집혔듯, 실사용 없이 쓴 규칙은 뒤집힌다. **구현(01·03~07)을 다 마친 뒤** 도그푸딩(이슈 02): 회사·개인 프로젝트에서 스킬을 실제로 쓰고 L1→L2→모듈 드릴다운 + 영향범위 색칠 + 캔버스 직접 수정을 돌린다(2026-09-14 그릴 — 구 "이슈 슬라이스 전 0바퀴" 폐기, 03은 확정 라우팅 21행을 그대로 구현). 규칙의 유지·수정·제거는 **모인 도그푸딩 이슈를 보고 사용자가 판단**한다 — 횟수 기준 없음(2026-09-14 그릴, 구 "세 번 이상 교정" 폐기; 기록 방법은 이슈 02).
...
4. 이슈 슬라이스 — §3.7 항목이 곧 후보: 7-2 스냅샷 영속화 / 7-3 근거 검사 / 7-4 스킬 교체(shim 포함) / 7-5 포장(a 두 채널, b 매니페스트 생성, b·c 쓰기 폴더) / 7-6 frame / 7-8 세션별 캔버스 / 7-9 글꼴 기본값(05에 포함). 7-1 삭제됨, 7-7은 작업 없음. ~~도그푸딩 0바퀴가 슬라이스 전에 와야 한다는 원칙~~ — 2026-09-14 그릴로 폐기: 도그푸딩(이슈 02)은 01·03~07 구현 뒤 맨 마지막. frame 기술 검증은 이슈 05 첫 단계.
