...
- **미포함 참조 (2026-09-19 2차 검수 A5):** `--frame`으로 일부만 export하면 파일 밖 대상을 가리키는 참조는 "미포함"으로 남긴다. import 뒤 그 참조는 어떤 기존 그림에도 붙지 않는다 — 이름이 같은 그림이 있어도 연결하지 않는다. 참조는 frame 이름이 아니라 ID 기반이어야 하며, 이름 변경·복사에 견디는 식별 방법은 구현에서 정한다. 대상을 자동으로 함께 저장해 사용자 지정 범위를 넓히지 않는다. 미포함 참조를 따라갈 때의 안내·새로 그리기는 03 담당.
