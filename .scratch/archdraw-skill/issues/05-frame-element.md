...
> 기술 검증 완료(2026-09-20, 코드 확인): 프론트 `scene.ts`는 이미 frame을 스켈레톤 변환에서 제외하고 `restoreElements`에 그대로 넘긴다. frame은 프론트 수정 없이 그려지며, 막는 곳은 서버 타입 목록 `ExcalidrawElementType`(frame 없음) 하나다. 이 이슈의 프론트 수정은 아래 글꼴 초기값 한 줄뿐이다.
