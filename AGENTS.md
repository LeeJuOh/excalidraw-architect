...
AI 에이전트와 개발할 때, 텍스트만으로는 서로 같은 도메인·구조·동작을 이해하고 있는지 확인하기 어렵다. 이 프로젝트는 개발자와 에이전트가 **같은 그림을 함께 보고 고치면서 공통의 이해를 만드는 것**을 목표로 한다. 도메인 모델링에서 유비쿼터스 언어를 맞추듯, 그림을 매개로 개념과 관계를 맞춘다.
