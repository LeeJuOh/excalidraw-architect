...
- 툴을 남기고 서버가 스킬 폴더의 md를 읽어 돌려주기: 서버는 npm으로 따로 배포돼 스킬 경로를 알 수 없고, 서버→스킬 의존이 생겨 기각했다. (2026-09-24: 이제는 패키지가 스킬 폴더를 싣는다. 판단 본문은 이 의존을 받아들여 서버가 스킬 폴더를 읽는다 — [ADR-0014](0014-skill-body-as-mcp-prompt.md).)
